...
Tests are located in the `test/` directory and are run via `uv run pytest -v`
or `make test`. The test suite currently composes of unit tests only --
purpose-built samples are required for full integration tests and as such
integration tests are not planned.
