...
void bdt_prep(std::string in_file, std::string out_file) {
    ...
    TMVA::Factory *factory =
        new TMVA::Factory("sigma0_photon_bdt", output.get());
    TMVA::DataLoader *loader = new TMVA::DataLoader("dataset");

    TMVA::gConfig().GetVariablePlotting().fPlotFormat =
        TMVA::Config::VariablePlotting::kPDF;
    ...
    TCut preselection =
        "pfp_trk_shr_score < 0.5 && pfp_trk_shr_score >= 0 && trk_llrpid > -50 "
        "&& shr_length > -5 && shr_open_angle > -5";
    loader->PrepareTrainingAndTestTree(preselection, "");

    factory->BookMethod(loader, TMVA::Types::kBDT, "BDT");
    factory->BookMethod(loader, TMVA::Types::kBDT, "BDTG", "V:BoostType=Grad");
...
int main(int argc, char **argv) {
    ...
    desc.add_options()("help,h", "output help message")(
        "input,i", po::value<std::string>(), "input file name")(
        "output,o", po::value<std::string>(),
        "output file name")("presel,p",
                            po::value<std::string>()->default_value(
                                "pfp_trk_shr_score > -0.5 && trk_llrpid > -50 "
                                "&& shr_length > -5 && shr_open_angle > -5"),
                            "preselection cut");
