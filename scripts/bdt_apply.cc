...
static const std::vector<std::string> branch_names = {
    "reco_primary_vtx_x", "reco_primary_vtx_y",  "reco_primary_vtx_z",
    "pfp_trk_shr_score",  "trk_llrpid",          "shr_start_x",
    "shr_start_y",        "shr_start_z",         "shr_length",
    "shr_open_angle",     "pfp_photon_bdt_score"};
...
void bdt_apply(std::string in_file, std::string weights_file) {
    Double_t reco_primary_vtx_x, reco_primary_vtx_y, reco_primary_vtx_z;
    ...
    if (!t_Output || t_Output->IsZombie()) {
        throw std::runtime_error(
            "ERROR: could not get ana/OutputTree from input file");
    }
    ...
    Float_t _pfp_trk_shr_score, _trk_llrpid,
        /* _trk_three_plane_dedx, */
        _shr_length, _shr_open_angle, _nusep;
    ...
    /// NOTE: use a raw pointer here as std::unique_ptr clashes with ROOTs
    /// memory mgmt.
    TBranch *bdt_branch =
        t_Output->Branch("pfp_photon_bdt_score", &pfp_photon_bdt_score);
    /* std::unique_ptr<TBranch>
     * bdt_branch(t_Output->Branch("pfp_photon_bdt_score",
    ...
    for (Long64_t i_evt = 0; t_Output->LoadTree(i_evt) >= 0; ++i_evt) {
        ...
        TVector3 nu_vertex(reco_primary_vtx_x, reco_primary_vtx_y,
                           reco_primary_vtx_z);
        ...
        for (size_t i_pfp = 0; i_pfp < pfp_trk_shr_score->size(); ++i_pfp) {
            ...
            TVector3 shr_start(shr_start_x->at(i_pfp), shr_start_y->at(i_pfp),
        ...
        if (i_evt % 100 == 0)
            std::printf("%lld / %lld (%.1f %%) events processed\n", i_evt,
                        n_entries,
                        100.0 * (static_cast<float>(i_evt) /
                                 static_cast<float>(n_entries)));
    }
...
int main(int argc, char **argv) {
    ...
    desc.add_options()("help,h", "output help messaege")(
        "input,i", po::value<std::string>(), "input file name")(
        "weights,w",
        po::value<std::string>()->default_value(
            "./dataset/weights/sigma0_photon_bdt_BDT.weights.xml"),
    ...
    if (vm.count("input") && vm.count("weights")) {
        bdt_apply(vm["input"].as<std::string>(),
                  vm["weights"].as<std::string>());
    } else {
